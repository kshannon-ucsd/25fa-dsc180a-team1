...
query = f"""
CREATE MATERIALIZED VIEW mimiciii.filtered_patients_agegrouped_with_morbidities_and_elective_flag AS
 SELECT c.subject_id,
    c.hadm_id,
    c.age,
        CASE
        ...
        END AS age_bin,
        CASE
            WHEN (upper(TRIM(BOTH FROM c.admission_type)) = 'ELECTIVE'::text) THEN 1 
            ELSE 0
        END AS admission_elective_flag,
...
selection_query = f"""
SELECT * from mimiciii.filtered_patients_agegrouped_with_morbidities LIMIT 1;
"""

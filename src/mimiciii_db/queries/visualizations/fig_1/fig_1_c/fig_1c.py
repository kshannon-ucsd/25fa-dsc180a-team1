#!/usr/bin/env python3
"""
Generate network visualization for comorbidity relationships (Fig 1c).
Creates a network graph showing comorbidity relationships based on the Elixhauser Index.
...
OUTPUT_PATH = repo_root / "assets" / "fig_1" / "fig_1c.png"


def get_detailComorbidityData_PerPatient(db_conn):
    ...
    """
    admission_comorbidity_details = db_conn.query_df("SELECT * FROM elixhauser_quan")
    target_patients = db_conn.query_df("SELECT * FROM filtered_patients")
...
def main():
    """Main function to generate the network visualization."""
    # Connect to database
    db_conn = DB.from_url(db_url())

    # Getting data
    df_tpatients_comorbidity_details = get_detailComorbidityData_PerPatient(db_conn)
    ...
    build_network_graph(edge_data, prevalence_norm, 25000, 1.25, str(OUTPUT_PATH))
